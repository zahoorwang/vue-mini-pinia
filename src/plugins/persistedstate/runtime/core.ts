...
function hydrateStore(store: Store, { storage, serializer, key, debug, pick, omit, beforeHydrate, afterHydrate }: Persistence, context: PiniaPluginContext, runHooks = true) {
  try {
...
function persistState(state: StateTree, { storage, serializer, key, debug, pick, omit }: Persistence) {
  try {
